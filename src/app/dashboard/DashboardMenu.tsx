...
export default function DashboardMenu() {
    ...
    return (
        <div 
            className='flex bg-stone-400 dark:bg-sky-950 border-r-4 border-slate-500 h-full min-h-screen overflow-hidden w-full p-2 flex-col'
        >
                ...
                {user &&
                    <>
                        ...
                        <span className='text-center'>
                            ...
                            <h3 className='text-lg font-serif text-green-600 dark:text-green-600'>{user.role}</h3>
                        </span>
                    </>
                }
            </div>
            <h2 className="border-y-3 border-slate-500 text-sm lg:text-lg font-bold text-center bg-transparent h-12 rounded-xs flex flex-row justify-center gap-4 items-center uppercase tracking-widest">
            ...
            <ul className="mt-4 uppercase tracking-widest font-bold text-slate-900 text-sm">
                {user?.access.includes('users') && 
                    <li className={`transition-all duration-200 cursor-pointer w-full my-1 h-12 p-2 flex flex-row justify-between items-center hover:bg-slate-400 ${showingComponents.includes('users') ? 'bg-slate-300 shadow-none ' : 'bg-white/30  '}`}>
                        <button 
                ...
                {user?.access.includes('contacts') &&                 
                    <li className={`transition-all duration-200 cursor-pointer w-full my-1 h-12 p-2 flex flex-row justify-between items-center hover:bg-slate-400 ${showingComponents.includes('contacts') ? 'bg-slate-300 shadow-none ' : 'bg-white/30 dark:bg-white/40  '}`}>
                        <button 
                            className='inline-flex gap-2 items-center w-full justify-between cursor-pointer h-full'
                ...
                {user?.access.includes('tasks') &&                     
                    <li className={`transition-all duration-200 cursor-pointer w-full my-1 h-12 p-2 flex flex-row justify-between items-center hover:bg-slate-400 ${showingComponents.includes('tasks') ? 'bg-slate-300 shadow-none ' : 'bg-white/30 dark:bg-white/40  '}`}>
                        <button 
                            className='inline-flex gap-2 items-center w-full justify-between cursor-pointer h-full'
                ...
                {user?.access.includes('calendar') && 
                    <li className={`transition-all duration-200 cursor-pointer w-full my-1 h-12 p-2 flex flex-row justify-between items-center hover:bg-slate-400 ${showingComponents.includes('calendar') ? 'bg-slate-300 shadow-none ' : 'bg-white/30 dark:bg-white/40  '}`}>
                        <button 
                            className='inline-flex gap-2 items-center w-full justify-between cursor-pointer h-full'
                ...
                {user?.access.includes('monetization') &&                 
                    <li className={`transition-all duration-200 cursor-pointer w-full my-1 h-12 p-2 flex flex-row justify-between items-center hover:bg-slate-400 ${showingComponents.includes('money') ? 'bg-slate-300 shadow-none ' : 'bg-white/30 dark:bg-white/40  '}`}>
                    <button 
                            className='inline-flex gap-2 items-center w-full justify-between cursor-pointer h-full'
                ...
                {user?.access.includes('faq') && 
                    <li className={`transition-all duration-200 cursor-pointer w-full my-1 h-12 p-2 flex flex-row justify-between items-center hover:bg-slate-400 ${showingComponents.includes('faq') ? 'bg-slate-300 shadow-none ' : 'bg-white/30 dark:bg-white/40  '}`}>
                        <button 
                            className='inline-flex gap-2 items-center w-full justify-between cursor-pointer h-full'
